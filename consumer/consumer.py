...
from analyzer.analyzerPaper import analyzerPaper
from analyzer.analyzerProject import analyzerProject
...
class Consumer(threading.Thread):
    ...
    def __init__(self, site):
        ...
        bootstrap_servers = [config["IP"] + ":9092"]
    ...
    def processing_rate(self, data, keyId):
        ...
        if (self.pre_progress[keyId] + 1 <= raw_progress ) and (raw_progress != 100) or self.pre_progress[keyId] > raw_progress:
            ...
            self.dbs['QueryKeyword'].update({"_id": keyId}, {'$set': {"progress": float(raw_progress)}})
